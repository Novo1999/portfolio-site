'use client'
import { getResumeUrl } from '@/actions.js'
...
const HeroSection = () => {
  ...
  useEffect(() => {
    getResumeUrl().then((res) => setResumeUrl(res.resumeUrl))
  }, [])
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 relative md:top-6">
        <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="col-span-7 place-self-center text-center sm:text-left">
          <h1 className="text-white mb-4 text-4xl sm:text-5xl lg:text-6xl font-extrabold md:absolute top-0">
            {' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-green-600">Hello, I&apos;m </span>
            <br />
            <TypeAnimation sequence={['Novo', 1000, 'Frontend Developer', 1000, 'MERN Developer', 1000, 'React Developer', 1000, 'Next.JS Developer']} wrapper="span" speed={50} repeat={Infinity} />
          </h1>
          <p className="text-gray text-base sm:text-lg lg:text-xl md:mt-20 mb-6">A Frontend focused developer who can also build backends using express</p>
          <div>
            {/* Button */}
            <button
              onClick={(e) => handleClickToScroll(e, sectionRefs['contact'])}
              className="px-6 py-3 w-full sm:w-fit rounded-full mr-4 bg-gradient-to-br cursor-pointer from-teal-500 via-blue-500 to-green-500 text-white bg-white hover:bg-slate-200 "
            >
              Hire Me
            </button>

            <button className="px-1 py-1 w-full relative sm:w-fit rounded-full bg-transparent cursor-pointer bg-gradient-to-br from-blue-500 via-teal-500 to-green-600 hover:bg-slate-800 text-white border mt-3">
              <Link target="_blank" href={resumeUrl} className="block bg-[#121212] !cursor-pointer hover:bg-slate-800 rounded-full px-5 py-2 " download>
                View CV
              </Link>
            </button>
          </div>
